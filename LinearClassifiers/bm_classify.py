...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for _ in range(max_iterations+1):
            s = np.zeros(D)
            bs = 0
            predict = np.dot(X, w.T) + b
            for i in range(len(predict)):
                predict[i] = 0 if predict[i] <= 0 else 1
            for i in range(N):
                if predict[i] != y[i]:
                    if y[i] == 0:
                        s -= X[i]
        ...
        for _ in range(max_iterations):
            predict_X = np.dot(X, w.T) + b
            P = sigmoid(predict_X) - y
            w -= step_size / N * np.dot(P, X)
            b -= step_size / N * np.sum(P)
...
def multiclass_train(X, y, C,
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            ####################################################
            predict = np.dot(X[n], w.T) + b
            stable_predict = predict - np.amax(predict)
            softmax = np.exp(stable_predict) / np.sum(np.exp(stable_predict), axis=0)
            yonehot = np.zeros(C)
            yonehot[y[n]] = 1
            P = softmax - yonehot
            w -= step_size  * np.dot(P[:,None], X[n][None,:])
            b -= step_size  * P
        ...
        for _ in range(max_iterations):
            predict = np.dot(X, w.T) + b
            stable_predict = predict - np.amax(predict)
            softmax = np.exp(stable_predict) / np.sum(np.exp(stable_predict), axis=1)[:, None]
            yonehot = np.zeros((N, C))
            for j in range(N):
                yonehot[j, y[j]] = 1
            P = softmax - yonehot
            w -= step_size / N * np.dot(P.T, X)
            b -= step_size / N * np.sum(P, axis=0)
